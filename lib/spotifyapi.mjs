...
import { PlayPlayLicenseRequest, PlayPlayLicenseResponse } from "../playplay/proto.mjs";
...
export default class SpotifyApi {
	...
	static async getPlayPlayLicense(challenge, fileId) {
		const req = await this.#fetchWithToken(
	...
	static #REQUEST_TOKEN = Buffer.from("01e132cae527bd21620e822f58514932", "hex");

	// Thanks SpotifyDL
	static async getPlayPlayKey(fileId) {
		const playplayLicenseRequest = PlayPlayLicenseRequest.encode({
			version: 2,
			token: SpotifyApi.#REQUEST_TOKEN,
			interactivity: 1,
			content_type: 1
		});

		const playplayResponseBytes = await SpotifyApi.getPlayPlayLicense(playplayLicenseRequest.finish(), fileId);

		const response = PlayPlayLicenseResponse.decode(playplayResponseBytes);

		if(!response.obfuscated_key)
			throw new Error("No obfuscated key in PlayPlay license response");

		const key = deobfuscateKey(Buffer.from(fileId, "hex"), response.obfuscated_key);
