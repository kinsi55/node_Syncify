...
export default class SpotifyApi {
	...
	static refreshTokenIfNecessary() {
		...
		return pRetry(async() => {
			try {
				...
				console.log("Failed to get Accesstoken:", ex);
			}
		});
	}

	static async getAccessToken() {
		const {serverTime: _serverTime} = await fetch("https://open.spotify.com/api/server-time").then(x => x.json());
		const serverTime = _serverTime * 1000;

		const params = new URLSearchParams();
		params.append("reason", "init");
		params.append("productType", "web-player");
		params.append("totpVer", TOTP.version);
		params.append("ts", serverTime);
		params.append("totp", TOTP.generate(serverTime));

		const cookie = await readFile("cookie.txt");

		const req = await fetch(`https://open.spotify.com/api/token?${params}`, {
			headers: {
				cookie: `sp_dc=${cookie}`
			}
		});
	...
	static getWidevinceLicenseUrl(type = "audio") {
		return `https://gue1-spclient.spotify.com/widevine-license/v1/${type}/license`;
	}

	static async getWidevineLicense(challenge, type = "audio") {
		const req = await this.#fetchWithToken(
			this.getWidevinceLicenseUrl(type),
			{
				method: "post",
				body: challenge
			}
	...
	static async getWidevineDecryptionKey(pssh) {
		if(SpotifyApi.#WVD_Device === null) {
			...
			if(config.wvd?.startsWith("http")) {
				const decryptorRequest = await fetch(config.wvd, {
					body: JSON.stringify({
						license: this.getWidevinceLicenseUrl(),
						pssh,
						cache: true,
						headers: [
							""
		...
		const cdmSession = new CDM(SpotifyApi.#WVD_Device);
