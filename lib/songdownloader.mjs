...
export class SongDownloader {
	...
	static async downloadSong(trackId, { dirName = null, fileNameOverride, skipDuplicate = true } = {}) {
		...
		if(config.download.format !== "MP4_128")
			throw new Error("Only MP4_128 is supported for free accounts");

		if(config.download.downloadCovers)
			throw new Error("Album Cover downloading is not implemented");
		...
		const tags = {
			...
			comment: config.download.tagSongUrl && `https://open.spotify.com/intl-de/track/${trackId}`
		};
		...
		await SongDownloader.remux(decryptionKey.key, songBuffer, fullPath, tags);
	...
	static async remux(key, encryptedSong, savePath, tags = {}) {
