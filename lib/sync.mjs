...
import { PlaylistDownloader } from "./playlistdownloader.mjs";
		...
		for(const payload of parsed.payloads) {
			...
			for(const op of msg.ops) {
				if(op.add) for(const item of op.add.items) {
					PlaylistDownloader.songAddedToPlaylist(playlistId, item.uri.split(":").pop());
				}

				if(op.rem) for(const item of op.rem.items) {
					PlaylistDownloader.songRemovedFromPlaylist(playlistId, item.uri.split(":").pop());
				}
			}
		}
	}
